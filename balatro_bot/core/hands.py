...
#:
#: **Сверены с игрой 2026-09-05** (закрытое допущение PLAN.md §8.3 №1).
#: Источник — таблица `G.GAME.hands` в `game.lua` (строки 2002–2013
#: `Balatro.love`): поля `chips`/`mult` каждого типа. Сверка механическая,
#: все 12 пар совпали, расхождений нет. Числа были выписаны по памяти и
#: оказались верны — но проверялись, а не подтверждались памятью повторно.
...
#: Прибавка за каждый уровень руки выше первого. Сверена там же и тогда же,
#: что и `BASE_VALUES`: поля `l_chips`/`l_mult` той же таблицы `game.lua`,
#: все 12 пар совпали.
...
HAND_VALUES_ARE_PROVISIONAL: Final[bool] = False
"""Флаг для механизма честности: сверены ли значения рук с игрой.

Снят 2026-09-05. Оказалось, что сверка не требует ни запущенной игры, ни
генератора: `Balatro.love` — обычный zip, а `game.lua` содержит таблицу
`G.GAME.hands` целиком. Все 24 числа обеих таблиц выше сверены и совпали,
поэтому расчёт больше не помечается неточным из-за них. Числа при этом
остаются выписанными вручную — их держит тест
`tests/test_hands.py::TestЗначенияРук`, а не генерация.
"""
