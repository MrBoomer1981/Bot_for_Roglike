...
def _apply_boss_score_modifier(ctx: ScoreContext) -> None:
    ...
    **Сверено с игрой 2026-09-05** (закрытое допущение PLAN.md §8.3 №2).
    Формула — `blind.lua`'s `Blind:modify_hand`, а вызывается она в
    `functions/state_events.lua` сразу после чтения базовых значений руки и
    **до того, как посчитана хоть одна карта или джокер**. То есть
    уполовинивается ровно база, а всё, что прибавляется дальше, идёт в полную
    силу — это в точности то, что делает шаг 1.5 нашего конвейера.

    Раньше здесь стояло `mark_unknown` при любом джокере в раскладке: из
    предположения, что в игре уполовинивание происходит где-то посреди
    джокеров и наш порядок его не воспроизводит. Чтение исходника это
    предположение опровергло — порядок совпадает, и пометка была лишней,
    занижая точность каждой руки под этим боссом.

    `Chicot` отключает боссовый блайнд целиком, и `modify_hand` тогда
    начинается с `if self.disabled then return mult, hand_chips, false end` —
    уполовинивания не происходит вовсе.
    """
    blind = ctx.state.blind
    if blind is None or blind.name != _FLINT_BOSS_NAME:
        return
    if ctx.modifiers.chicot:
        ctx.trace.append(
            TraceStep(
                _FLINT_BOSS_NAME, "блайнд отключён Chicot — счёт не тронут", ctx.chips, ctx.mult
            )
        )
        return
...
def _run_once(
    ...
    if hand_info is not None and hand_info.level > 1:
        # Уровень поднят Planet-картами — показываем его, иначе большой
