...
class GameState:
    ...
    `SHOP`). Нужна для `solver/shop.py`: показать рядом с оценкой текущего
    предложения, не для вердикта «рероллить или нет» — честная цена
    случайной альтернативы потребовала бы таблицы шансов генерации
    магазина, которую бот не строит (см. докстринг `solver/shop.py`)."""
