...
#: — это секунды, а с `BALATROBOT_FAST=1` куда быстрее.
#:
#: Величина подпёрта замерами, а не запасом «на всякий случай». За 38 888
#: ответов мода максимум законного — 9192 мс (`play`), в полосе 10–90 с нет ни
#: одного ответа, а за 90 с — ровно два зависших. Отсюда 45 с: они лежат внутри
#: пустой полосы, то есть уверенно отделяют «зависло» от «долго считает», и при
#: этом вдвое дешевле прежних 90 с, которые ран просто терял.
#:
#: Почему не 30 с, хотя запас над 9192 мс это позволяет: замеры сняты при
#: `GAMESPEED` >= 4 (умолчание мода — 4, `BALATROBOT_FAST=1` — 10). Если игру
#: запустят с `GAMESPEED=1`, та же анимация растянется примерно вчетверо, до
#: ~37 с, и 30 с оказались бы тесны. По `BALATROBOT_GAMESPEED` таймаут НЕ
#: масштабируем: переменная говорит, что просили мы, а игру могли поднять
#: руками с другой.
