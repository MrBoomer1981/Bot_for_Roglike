...
#: Статусы блайнда, означающие «играть его уже не придётся». Тот же список,
#: что в `autopilot._BLIND_DONE_STATUSES`, и по той же причине: пропущенный
#: ради тега блайнд не будет сыгран, хотя и не побеждён. До улучшения A14
#: скипов не бывало вовсе, поэтому фильтр по одному `DEFEATED` был верен;
#: с включёнными скипами он завышал горизонт (ран 13).
_BLIND_DONE_STATUSES: Final[frozenset[str]] = frozenset({"DEFEATED", "SKIPPED"})


def _rounds_left_in_ante(state: GameState) -> int | None:
    """Сколько раундов этого анте ещё предстоит сыграть — горизонт для
    денежных ваучеров второго уровня, взятый из самого состояния, а не
    выдуманный.

    «Предстоит» — не то же, что «не побеждён»: скипнутый блайнд играть уже
    не придётся, и считать его в горизонте значило бы завысить доход
    ваучера. `None`, если область `blinds` не пришла вовсе (ручной ввод) —
    тогда честно нечего посчитать, а не ноль или произвольная константа."""
    if not state.blinds:
        return None
    return sum(1 for blind in state.blinds.values() if blind.status not in _BLIND_DONE_STATUSES)
