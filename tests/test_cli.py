...
class TestAdvise:
    ...
    def test_сброс_слишком_много_комбинаций_честно_отказывается(
        ...
        # Без suit-джокера этот же сброс (C(44, 3) = 13244 сырых доборов)
        # теперь считается точно: масть некому различать, а флеш недостижим
        # в пределах трёх карт добора, поэтому перебор схлопывается всего до
        # ~455 композиций по рангам (см. `solver/discard.py`). Greedy Joker
        # различает конкретную масть (бубны) — это форсирует полную
        # детализацию по масти, а в стандартной колоде каждая карта и так
        # уникальна по (ранг, масть), так что схлопывать становится нечего,
        # и честный отказ по лимиту композиций возвращается снова.
        код = cli.main(
            [
                "advise",
                "--hand",
    ...
    def test_сброс_трёх_карт_теперь_считается_точно(
        self, capsys: pytest.CaptureFixture[str]
    ) -> None:
        # Раньше C(44, 3) = 13244 сырых доборов превышало старый лимит и
        # честно отказывалось; сжатый перебор по классам эквивалентности
        # (см. `solver/discard.py`) схлопывает это до ~455 композиций по
        # рангам, укладывается в лимит и теперь считает точно.
        код = cli.main(["advise", "--hand", "AH KH QH JH 9H 7C 7D 2S", "--discard", "7C 7D 2S"])
        out = capsys.readouterr().out
        assert код == 0
        assert "перебор добора недоступен" not in out
        assert "переборано доборов: 13244" in out

    def test_поиск_лучшего_сброса_по_флагу(self, capsys: pytest.CaptureFixture[str]) -> None:
        # Маленькая рука (3 карты разных мастей) — чтобы честный перебор всех
        # наборов сброса (`rank_discards`) не растянул тест: флеш здесь
        # физически недостижим ни при каком сбросе, значит масть везде
        # схлопывается и перебор остаётся дешёвым.
        код = cli.main(
            [
                "advise",
                "--hand",
        ...
        out = capsys.readouterr().out
        assert код == 0
        assert "что выгоднее сбросить" in out
