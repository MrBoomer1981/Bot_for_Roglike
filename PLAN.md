...
fix, stake stickers, run-runner) plus improvements A1–A5, B1, D1 and F1 from section 9.8 — see
section 6, "Autopilot" subsection. Run 6 (RED/WHITE) was the first full live run of the 9.1–9.7 loop — it reached ante 5 (a new
record) and was stopped manually to start on the fixes it surfaced; those are now done (B1
over-aggressive discarding, A5 shop reroll, F1 the 15–40 s → ~0.6 s decision loop). What's
left: a live shakedown run on a Mac, the smaller shop-policy fixes (A6/A7), Tarot consumables
(C1), Arcana/Spectral/Standard packs, an unattended 24/7 mode, and the mass win-rate
...
B1 (over-aggressive discarding), A5 (shop reroll) and F1 (decision-loop speed) are now closed
on top of those: the economy engine is complete (buy / sell-replace / voucher / pack / reroll)
and a `SELECTING_HAND` decision is ~0.6 s instead of 15–40 s. What's left before the batch
win-rate run (E1) is a live shakedown on a Mac and the smaller polish items (A6/A7, Tarot).
...
- **E1/E2. Mass win-rate measurement on a live Mac → 24/7 mode.** First real progress: run 6
  ...
  the boss-debuff path all worked live for the first time. B1, A5 and F1 are all closed now;
  no code blocker remains. Next: a live shakedown run on a Mac to check the A5 reroll policy,
  the B1 guards and F1's surrogate order-search against the real game, then the batch itself.
