...
**Сделано (первый, узкий срез — только фаза `SELECTING_HAND`).** `balatro_bot/autopilot.py`:
...
Любая другая фаза (`BLIND_SELECT`, `SHOP`, `*_PACK`, ...) намеренно возвращает `None` — решения
там ещё не замкнуты (9.2–9.4 ниже), автопилот на них ничего не делает, ровно как `watch`.
...
не покрывает) не роняет цикл — печатает ошибку и продолжает опрос тем же состоянием. Тесты —
`tests/test_autopilot.py`, `tests/test_tui.py::TestAutoplay`. **Вживую на Mac не проверялось** —
пока только на фальшивом моде (`tests/fake_mod.py`), который не симулирует реальный розыгрыш.
...
| 9. Автопилот | подзадача 9.5 («Правило-модифицирующие боссы») **закрыта полностью** — каталог (`core/bosses.py`), починка счёта под `The Flint` и фильтр нелегальных ходов в `solver/play.py`; подзадача 9.1 — **первый узкий срез сделан**: `balatro_bot/autopilot.py` + `ui/tui.py.autoplay()` (`balatro-bot autoplay`), розыгрыш/сброс через `ModBridge.play`/`.discard` по топ-1 `rank_actions`, переключатель «пауза/перехват» на клавишу `p` между каждым действием — вживую на Mac не проверялось, только на `tests/fake_mod.py` | 9.1 — остальные фазы цикла (магазин, выбор блайнда, паки, ...) всё ещё просто `None`, ждут 9.2–9.4; 9.2 (замыкание решений без вердикта), 9.3 (ваучеры/паки), 9.4 (консумабли), 9.6 (ставки), 9.7 (ран-раннер) не начаты |
