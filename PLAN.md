...
| 5. Автоподключение | мост проверен вживую (`balatro-bot doctor` читает реальное состояние точно); `ui/tui.py` написан — `balatro-bot watch` сам перерисовывает советы при смене состояния, покрыт тестами на фальшивом моде | сам `watch`-цикл вживую ещё не запускался — только на фальшивом моде в тестах, это следующая проверка на Mac |
