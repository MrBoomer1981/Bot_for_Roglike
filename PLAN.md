...
| 9. Автопилот | подзадача 9.5 («Правило-модифицирующие боссы») **закрыта полностью** — каталог (`core/bosses.py`), починка счёта под `The Flint` и фильтр нелегальных ходов в `solver/play.py`; подзадача 9.1 (цикл действий) закрывает розыгрыш/сброс (`SELECTING_HAND` → `ModBridge.play`/`.discard`) и переключатель «пауза/перехват» на клавишу `p`; **9.2 закрыта полностью** — скип блайнда (`BLIND_SELECT` → `.select`/`.skip`), сбор награды (`ROUND_EVAL` → `.cash_out`), покупка джокеров в магазине (`SHOP` → `.buy`/`.next_round`, консервативная политика поверх `evaluate_shop`) и вскрытие Celestial/Planet Pack (`PLANET_PACK` → `.open_pack`, `solver/pack.py`, политика без консерватизма — подъём уровня руки не может ухудшить счёт) — вживую на Mac не проверялось, только на `tests/fake_mod.py`; **9.3 — первые два уровня из трёх сделаны** (`solver/vouchers.py`): точный расчёт (`Grabber`/`Nacho Tong`/`Paint Brush`/`Palette`/`Wasteful`/`Recyclomancy`) и денежная формула с явным горизонтом (`Seed Money`/`Money Tree`/`Reroll Surplus`/`Reroll Glut`/`Clearance Sale`/`Liquidation`) — заодно добавлен `GameState.used_vouchers` (мост раньше не парсил эту область мода вовсе) и `core/economy.py`, что попутно закрыло старое допущение `JokerOffer.interest_lost` о потолке процентов $25 по умолчанию. `ShopAdvice.vouchers` несёт все эти оценки, но автопилот их пока не покупает — только `evaluate_shop`/джокеры входят в `_decide_shop_action` | остальные фазы цикла (Tarot/Spectral/Standard/Buffoon-паки, ...) всё ещё просто `None` в `decide_action`; 9.3 — третий уровень (эвристическая константа) и `Hieroglyph`/`Petroglyph` (нужна формула требований блайнда по анте, которой нет вовсе) не сделаны; автопилот не покупает ваучеры даже из уже оценённых; паки (кроме Celestial/Planet) не оценены; 9.4 (консумабли), 9.6 (ставки), 9.7 (ран-раннер) не начаты |
