...
«считаем/не считаем», а по трём уровням, согласно новой третьей категории честности выше:
  ...
  **Паки — Celestial и Buffoon сделаны.** Магазин показывает только тип и цену; содержимое
  ...
  - Celestial-пак *в витрине* (покупка) пока не оценивается — можно тем же приёмом, что Buffoon
    (матожидание по 12 планетам), отдельным куском.
...
- **D1. Перестановка джокеров.** Автопилот играет джокеров в порядке слотов;
  `rank_joker_orders` не вызывается (в отличие от `watch`). После покупки/продажи
  прогнать перебор порядка → `ModBridge.rearrange`. Дёшево, усиливает A1.
- **A3. Покупка Celestial-пака из витрины.** Тот же приём, что Buffoon (Монте-Карло по
  12 планетам). Денежный сток, масштабирующийся в большие блайнды.
- **A2. Строже порог покупки джокера.** `uplift > 0` при шумной оценке по 12 рукам берёт
  мусор в слот навсегда — возможно, не нужно после A1.
- **A4. Покупка ваучеров.** `evaluate_vouchers` уже считает три уровня, мост
  `buy(voucher=…)` готов — нужен вердикт, может ли `heuristic_value` двигать автопокупку.
