...
| 8. Стратегия рана | скип блайнда (`solver.skip.evaluate_skip`, `doctor`/`watch`) и экономика (проценты — Фаза 7; момент рерола — `ShopAdvice.reroll_cost`, честно без вердикта) сделаны; советы по покупкам переехали в Фазу 7 (см. её строку выше) как относящиеся именно к магазину | полная симуляция ранов не начата — подробности раздел 6, «Стратегия рана» |
