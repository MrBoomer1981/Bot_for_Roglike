...
**Фильтр нелегальных вариантов в `solver/play.py`** — осталось несделанным. Нужен для трёх
`restricts_legal_plays`-боссов (`The Mouth`/`The Eye`/`The Psychic`) — до того, как нелегальный
вариант попадёт в ранжированный список, не после того, как автопилот попробует его сыграть и
получит отказ. Для `The Mouth`/`The Eye` нужна история сыгранных в этом раунде типов руки —
`PokerHandInfo.played_this_round` уже есть в состоянии (добавлено ради `j_card_sharp`, раздел
8.3 №3), фильтр — вопрос применения, не сбора данных.
...
| 9. Автопилот | **не начата** в целом — новая фаза этой редакции плана, объём под новую цель (раздел 2); из подзадачи 9.5 сделаны каталог (`core/bosses.py`, 28 боссов) и починка счёта под `The Flint` (`core/scoring.py`) | 6 из 7 подзадач раздела 6 «Автопилот» целиком: цикл действий, замыкание решений без вердикта, ваучеры/паки, вскрытие паков, консумабли перед розыгрышем, ставки (Perishable/Rental), ран-раннер; из 9.5 — фильтр `restricts_legal_plays`-боссов в `solver/play.py` |
...
| 10 | **Каталог сделан** (`core/bosses.py`, 28 боссов из `game.lua`/`blind.lua`/`functions/state_events.lua`, раздел 6 «Автопилот» п. 9.5). **`The Flint` — починено**: `core/scoring.py._apply_boss_score_modifier` уполовинивает базовые фишки/множитель (`floor(x*0.5+0.5)`, минимум 1/0), формула из `blind.lua`'s `Blind:modify_hand`. Осталась одна честно признанная неполнота: наш конвейер считает джокеров последним шагом (раздел 5), а в игре The Flint срабатывает до хендовых джокеров и после карт — точно воспроизвести это разделение нельзя, поэтому расчёт точен только без джокеров в раскладке, а с любым джокером честно помечается `unknown` (не тихо неверное число). Осталось несделанным: солвер не фильтрует три `restricts_legal_plays`-босса (`The Mouth`/`The Eye`/`The Psychic`) — движок может честно посчитать и порекомендовать ход, нелегальный в этом раунде | молчаливо неверный совет о легальности хода — хуже, чем `unknown`, для советника, и блокер для автопилота | фильтр в `solver/play.py` по `PokerHandInfo.played_this_round` (уже есть в состоянии) и `BossEffect.restricts_legal_plays` |
