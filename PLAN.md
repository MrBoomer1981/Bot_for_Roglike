...
Осталось два независимых куска работы:
1. **Фильтр нелегальных вариантов в `solver/play.py`** для трёх `restricts_legal_plays`-боссов
   — до того, как они попадут в ранжированный список, не после того, как автопилот попробует
   их сыграть и получит отказ. Для `The Mouth`/`The Eye` нужна история сыгранных в этом раунде
   типов руки — `PokerHandInfo.played_this_round` уже есть в состоянии (добавлено ради
   `j_card_sharp`, раздел 8.3 №3), фильтр — вопрос применения, не сбора данных.
2. **`The Flint` — реальный, а не только «нелегальный ход», дефект подсчёта**: базовые фишки и
   множитель руки уполовинены при подсчёте (`floor(x*0.5+0.5)`, минимум 1 у множителя и 0 у
   фишек — из `blind.lua`'s `Blind:modify_hand`), и это **нигде не реализовано в
   `core/scoring.py`** — при этом боссе бот сейчас завышает счёт примерно вдвое, а не просто
   рискует предложить нелегальный ход. Обнаружено при построении каталога, не было отдельным
   пунктом допущений раньше — задокументировано здесь, чтобы не потерять.
...
| 9. Автопилот | **не начата** в целом — новая фаза этой редакции плана, объём под новую цель (раздел 2); из подзадачи 9.5 сделан только каталог (`core/bosses.py`, 28 боссов) | 6 из 7 подзадач раздела 6 «Автопилот» целиком: цикл действий, замыкание решений без вердикта, ваучеры/паки, вскрытие паков, консумабли перед розыгрышем, ставки (Perishable/Rental), ран-раннер; из 9.5 — фильтр `restricts_legal_plays`-боссов в `solver/play.py` и починка счёта под `The Flint` |
...
| 10 | **Каталог сделан** (`core/bosses.py`, 28 боссов из `game.lua`/`blind.lua`/`functions/state_events.lua`, раздел 6 «Автопилот» п. 9.5), но два куска работы из него ещё не сделаны: (a) солвер не фильтрует три `restricts_legal_plays`-босса (`The Mouth`/`The Eye`/`The Psychic`) — движок может честно посчитать и порекомендовать ход, нелегальный в этом раунде; (b) `The Flint` (базовые фишки и множитель уполовинены) нигде не реализован в `core/scoring.py` — это уже не риск нелегального хода, а прямое **завышение счёта примерно вдвое** при этом боссе | (a) молчаливо неверный совет о легальности хода; (b) молчаливо неверное число при живом `The Flint` — оба хуже, чем `unknown`, для советника, и блокер для автопилота | (a) фильтр в `solver/play.py` по `PokerHandInfo.played_this_round` (уже есть в состоянии) и `BossEffect.restricts_legal_plays`; (b) шаг подсчёта в `core/scoring.py`, применяющий `floor(x*0.5+0.5)` (минимум 1/0) к финальным фишкам/множителю, когда активен `The Flint` |
