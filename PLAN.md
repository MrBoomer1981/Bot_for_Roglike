...
| 3. Скоринг | движок готов, реализовано 36 джокеров из 150 | сверка чисел с игрой (Фаза 3a) |
...
| 5. Автоподключение | мост проверен вживую (`balatro-bot doctor` читает реальное состояние точно) | `ui/tui.py` не написан — советы пока не появляются сами по себе |
| 6. Сброс и EV | не начата, но разблокирована — Фаза 5 даёт точный состав колоды | нужна сама реализация; при первом подходе разумно начать с узкого среза (точный перебор для одного конкретного набора сбрасываемых карт), а не сразу с полного Монте-Карло по всем сбросам (см. допущение №9) |
...
| 3 | Реализовано 36 джокеров из 150 | остальные помечают расчёт неточным | Фаза 7 |
