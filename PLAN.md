...
Обе части — **вживую на Mac не проверялись**, пока только на фальшивом моде (`tests/fake_mod.py`,
который не симулирует ни реальный розыгрыш, ни реальный переход между блайндами).

Любая фаза, кроме `SELECTING_HAND`/`BLIND_SELECT` (`SHOP`, `*_PACK`, ...), по-прежнему намеренно
возвращает `None` — решения там ещё не замкнуты, автопилот на них ничего не делает, ровно как
`watch`.

**9.2. Замыкание решений без готового вердикта — осталось два места.** Скип блайнда сделан выше.
...
| 8. Стратегия рана | скип блайнда (`solver.skip.evaluate_skip`, `doctor`/`watch`) и экономика (проценты — Фаза 7; момент рерола — `ShopAdvice.reroll_cost`, честно без вердикта) сделаны — **все эвристики отдельных решений фазы закрыты**; советы по покупкам переехали в Фазу 7 (см. её строку выше) как относящиеся именно к магазину | полная симуляция ранов — прогон реального автопилота тысячи раз, не свой движок правил (уточнено 2026-08-27, см. раздел 6) — жёстко ждёт цикла действий автопилота, Фаза 9, ещё не начата |
| 9. Автопилот | подзадача 9.5 («Правило-модифицирующие боссы») **закрыта полностью** — каталог (`core/bosses.py`), починка счёта под `The Flint` и фильтр нелегальных ходов в `solver/play.py`; подзадача 9.1 (цикл действий) закрывает розыгрыш/сброс (`SELECTING_HAND`, топ-1 `rank_actions` → `ModBridge.play`/`.discard`) и переключатель «пауза/перехват» на клавишу `p`; из 9.2 сделан скип блайнда (`BLIND_SELECT`, `autopilot.decide_skip` — консервативная политика поверх `evaluate_skip`, → `ModBridge.select`/`.skip`) — вживую на Mac не проверялось, только на `tests/fake_mod.py` | из 9.2 остались магазин (купить/не купить, ваучеры/паки, реролл, момент ухода) и вскрытие паков; остальные фазы цикла (магазин, паки, ...) всё ещё просто `None` в `decide_action`; 9.3 (ваучеры/паки), 9.4 (консумабли), 9.6 (ставки), 9.7 (ран-раннер) не начаты |
