...
Все три части (`SELECTING_HAND`/`BLIND_SELECT`/`SHOP`+`ROUND_EVAL`) — **вживую на Mac не
проверялись**, пока только на фальшивом моде (`tests/fake_mod.py`, который не симулирует ни
реальный розыгрыш, ни реальный переход между фазами).

Любая фаза, кроме перечисленных выше (вскрытие паков, `*_PACK`), по-прежнему намеренно возвращает
`None` — решения там ещё не замкнуты, автопилот на них ничего не делает, ровно как `watch`.

**9.2. Замыкание решений без готового вердикта — осталось одно место.** Скип блайнда и покупка
джокеров в магазине сделаны выше.
...
| 9. Автопилот | подзадача 9.5 («Правило-модифицирующие боссы») **закрыта полностью** — каталог (`core/bosses.py`), починка счёта под `The Flint` и фильтр нелегальных ходов в `solver/play.py`; подзадача 9.1 (цикл действий) закрывает розыгрыш/сброс (`SELECTING_HAND` → `ModBridge.play`/`.discard`) и переключатель «пауза/перехват» на клавишу `p`; **9.2 закрыта почти полностью** — скип блайнда (`BLIND_SELECT` → `.select`/`.skip`), сбор награды (`ROUND_EVAL` → `.cash_out`) и покупка джокеров в магазине (`SHOP` → `.buy`/`.next_round`, консервативная политика поверх `evaluate_shop`) — вживую на Mac не проверялось, только на `tests/fake_mod.py` | из 9.2 осталось только вскрытие паков (`*_PACK`, не реализовано вовсе); остальные фазы цикла всё ещё просто `None` в `decide_action`; 9.3 (ваучеры/паки), 9.4 (консумабли), 9.6 (ставки), 9.7 (ран-раннер) не начаты |
